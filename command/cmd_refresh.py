...
def account_exists(email: str) -> bool:
    ...
    cur = conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS accounts (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT UNIQUE NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
...
class RefreshCommand(commands.Cog):
    ...
    @app_commands.command(
        name="refresh",
        description="Masukkan daftar Gmail (dipisah spasi atau enter)."
    )
    async def refresh(self, interaction: discord.Interaction, gmails: str):
        """
        Buyer mengetikkan Gmail langsung.
        Dicek dulu ke database: kalau tidak ada → NotFound.
        Kalau ada → lanjut request ke API.
        """
        ...
        headers = {"Content-Type": "application/json", "User-Agent": "Mozilla/5.0 (DiscordBot)"}
