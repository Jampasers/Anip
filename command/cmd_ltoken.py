...
# ----------------------------------
# KONFIG (DEKLARASI GLOBAL SEMENTARA)
# ----------------------------------
# Nilai akan diisi oleh os.getenv() di dalam fungsi setup()
API_KEY: str = "" 
BASE_URL: str = "https://cid.surferwallet.net/publicApi" # Default URL
...
# Cek sederhana agar bot tidak crash kalau env kosong
# Pengecekan ini dipindahkan ke setup
# ----------------------------------
...
async def http_get_json(url: str, *,
    ...
    async with aiohttp.ClientSession() as session:
        try:
            # Gunakan API_KEY yang sudah di set di setup()
...
class BuyLTokenModal(Modal, title="🛒 Buy LToken"):
    def __init__(self, c, conn, product_name: str):
        ...
        self.qty_input = TextInput(label=f"Amount ({product_name})", placeholder="1", required=True)
...
async def monitor_pending_orders_loop(bot: commands.Bot, c, conn) -> None:
    ...
    # Cek API_KEY lagi sebelum loop berjalan (sekaligus memastikan telah di set di setup)
    if not API_KEY:
...
def setup(bot: commands.Bot, c, conn, fmt_wl, PREFIX, DB_NAME=None): # <-- SETUP FIX
    ...
    # ----------------------------------
    # PINDAHAN KONFIG (PASTIKAN os.getenv() DIJALANKAN DI SINI)
    # ----------------------------------
    global API_KEY, BASE_URL # Wajib deklarasi global untuk mengubah nilai variabel global
    
    API_KEY = os.getenv("LTOKEN_API_KEY", "") 
    BASE_URL = os.getenv("LTOKEN_BASE_URL", "https://cid.surferwallet.net/publicApi")
    
    if not API_KEY:
        print("[CRITICAL] LTOKEN_API_KEY tidak ditemukan di .env. Fungsi LToken API akan error 400.")
    ...
    def build_ltoken_embed():
        """Bangun embed stok LToken berdasarkan data web."""
        async def inner():
    ...
    async def update_ltoken_stock():
        ...
        try:
            ...
            await message_cache["message"].edit(
                embed=embed,
                view=StockView(products, c, conn, 0, fmt_wl)
            )
        except Exception as e:
    ...
    async def ltokenstock(ctx: commands.Context):
        """Tampilkan embed stok LToken dari API, auto update setiap 10 detik"""
        if not API_KEY:
            await ctx.reply(
        ...
        msg = await ctx.send(
            embed=embed,
            view=StockView(products, c, conn, ctx.author.id, fmt_wl)
        )

        # Simpan untuk auto update
        message_cache["channel_id"] = ctx.channel.id
        message_cache["message"] = msg

        # Jalankan loop auto update kalau belum jalan
    ...
    async def order_status(ctx: commands.Context, order_id: str):
        if not API_KEY:
             await ctx.reply("❌ **ERROR**: LTOKEN_API_KEY belum diatur di file .env.", mention_author=False)
             return
