...
def setup(bot, c, conn, fmt_wl, PREFIX):
    @bot.command(usage=f'{PREFIX}addstock <code> "<title>" <item1,item2,...>  OR  {PREFIX}addstock <code> <item1,item2,...>')
    ...
    async def addstock(ctx, *, args: str):
        ...
        if '"' in args:
            try:
                ...
                await ctx.send("❌ Format salah. Gunakan kutip ganda untuk title. Contoh:\n"
